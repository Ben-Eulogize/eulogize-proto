...
// containerId: usually is the thumbnail list id
  ...
  return class WithSortableContainer extends React.Component {
    ...
    initContainer() {
      ...
      Promise.resolve(container).then((containerNode) => {
        ...
        Object.keys(this.events).forEach((key) =>
          events[key].forEach((eventName) => {
            if (this.prevContainer) {
              this.prevContainer.removeEventListener(
                eventName,
                this.events[key],
              )
            }
        ...
        if (this.prevContainer) {
          this.prevContainer.removeEventListener('keydown', this.handleKeyDown)
        }
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
      ...
      if (prevProps.containerId === props.containerId) {
        return
      }
      this.initContainer()
    }
      ...
      for (let i = 0, len = nodes.length; i < len; i++) {
        ...
        if (this.axis.x) {
          if (this.axis.y) {
            ...
            console.log(
              `[Bugsnag diagnostic data] - sortingOffset: ${JSON.stringify(
                sortingOffset,
              )}, windowScrollDelta: ${JSON.stringify(
                windowScrollDelta,
              )}, offset: ${JSON.stringify(
                offset,
              )}, edgeOffset: ${JSON.stringify(edgeOffset)}`,
            )
            if (
              mustShiftForward ||
              (index < this.index &&
